...
/* OFS에서 시작하여 FILE의 세그먼트를 주소 UPAGE에 로드합니다.
 * 총 READ_BYTES + ZERO_BYTES 바이트의 가상 메모리가 다음과 같이 초기화됩니다:
 *
 * - UPAGE에서 시작하는 READ_BYTES 바이트는 OFS에서 시작하는 FILE로부터 읽어와야 합니다.
 *
 * - UPAGE + READ_BYTES에서 시작하는 ZERO_BYTES 바이트는 0으로 초기화되어야 합니다.
 *
 * 이 함수에 의해 초기화된 페이지는 WRITABLE이 true이면 사용자 프로세스에 의해 쓰기가 가능해야 하며,
